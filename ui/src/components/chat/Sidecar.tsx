    ...
    <>
      ...
      <div className="shrink-0 px-4 py-3">
        ...
        <div className="h-1 overflow-hidden rounded-full bg-surface-hover">
          <div
            className={`h-full rounded-full transition-all duration-300 ${allDone ? "bg-accent" : "bg-claude"}`}
      ...
      <div className="shrink-0 border-b border-border px-4 py-3">
        <div className="flex items-center justify-between">
          {availableTabs.length > 1 ? (
            <div className="flex gap-1 rounded-md bg-surface-hover p-0.5">
              {availableTabs.map((tab) => (
                <button
                  key={tab.key}
                  onClick={() => setActiveTab(tab.key)}
                  className={`rounded px-2 py-0.5 text-[0.75rem] font-medium transition-colors ${
