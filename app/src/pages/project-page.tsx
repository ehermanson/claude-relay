...
import { BarChart3, ChevronDown, HelpCircle } from "lucide-react";
import { PageShell } from "@/components/ui/page-shell";
import { ProjectChatList } from "./chats-page";
...
export function ProjectPage() {
  ...
  // Overview IS the project's chat surface (no separate Chats tab on any
  // viewport). Project stats collapse into an expandable strip above the chat
  // list so we lead with chats but keep usage data a tap away. Identical on
  // mobile and desktop; only the chat rows themselves adapt in density.
  return (
    <PageShell maxWidth="wide">
      {hasContent && (
        <div className="mb-3 overflow-hidden rounded-lg border border-border/70 bg-surface">
          <button
            type="button"
            onClick={() => setStatsOpen((open) => !open)}
            className="flex w-full items-center justify-between px-3 py-2.5 text-[0.8125rem] font-medium text-muted transition-colors hover:bg-surface-hover"
          ...
          {statsOpen && <div className="border-t border-border/50 p-4">{statsContent}</div>}
        </div>
      )}
      <ProjectChatList />
    </PageShell>
